...
| `content/pages/pricing-preview.md` | `pricing-preview` | Net-new public **Pricing** page (`/pricing` at launch). Four plan tiers (Free / Basic $599 — *featured, "Most popular" badge + green border* / Team $999 / Enterprise Custom), a warm **EDL band**, a collapsible **plan-comparison table** (Product / Features / Data sets / Support groups + a merged "Contact sales" Enterprise column, driven by a small inline toggle script that resyncs the rowspan), a dark "how credits work" section, a 4-up access grid, `soc2_trust`, `faq_section` (adds FAQPage JSON-LD), and a **two-button final CTA built bespoke** (the shared `final_cta` is single-CTA). Implemented from the "Pricing Page v2" design on the refresh system — the design's fixed `1280px` framing was made **responsive**; asset paths mapped to served URLs. Plan cards / access cards are data-driven (`plans` / `access` lists); the featured tier is a `featured` flag. Also adds **Pricing to `header-refresh`** (flat link, left of Sign In, desktop + mobile) and **`footer-refresh`** (first Resources link). At launch: slug → `pricing`, drop `status: hidden`, net-new (no legacy file, no redirect) — **must promote coupled with the launch** since the nav/footer link `/pricing`. | Greenfield preview |
