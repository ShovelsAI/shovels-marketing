...
With the [Shovels API](https://shovels.redoc.ly/) in your toolbox, finding permits specific to a given address is a breeze. 
...
Using the [GET Search Addresses](https://shovels.redoc.ly/#operation/search_addresses_v1_addresses_search_get)  endpoint, search for the address to confirm that there are any permits on record (our system goes back ~20 years). 
...
Using the [GET Permits by Address](https://shovels.redoc.ly/#operation/get_permits_by_address_v1_permits_address_get) endpoint, enter the relevant address data to the next query. The list of required parameters are noted in the schema on the API doc linked just above. 
...
Using the [GET Permits by ID](https://shovels.redoc.ly/#operation/get_permits_by_id_v1_permits_get) endpoint, you can search for a single (or string array) of permit IDs. 
...
- A full list of our error codes are described in our API docs [here](https://shovels.redoc.ly/#section/API-Details). Most errors are 404s, which frequently mean either the address is input incorrectly, or there isn’t a permit record on file to match the search terms.
